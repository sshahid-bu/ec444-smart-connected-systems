...
static void infrared_range(void *args) {
  ...
  while (1) {
    ...
    slope = 39285; // calculated for linear approximation of distances >15cm
    distance = slope / voltage ;
    /*
    y = voltage (V)
    m = slope
    x = inverse number of distance (1/cm)
    b = intercept
    y = mx+b
    ( y - b ) / m = x
...
void app_main(void) {
  xTaskCreate(infrared_range, "infrared_range", 2048, NULL, configMAX_PRIORITIES-1, NULL);
}
