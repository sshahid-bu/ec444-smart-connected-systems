...
// Assignment:      EC444 Quest 1 Skill 11
    ...
    if(xQueueReceive(timer_queue, &evt, portMAX_DELAY)){
      if (evt.flag == 1) {
        if (gpio_get_level(BUTTON)) {
          time = -1; printf("button press, reset.\n");
        } 
        
        time = (time+1) % 100;
        printf("time: %d\n",time);

        displayBuffer[3] = intfonttable[time%10];
        displayBuffer[2] = intfonttable[time/10];
        ...
        i2c_master_stop(cmd4);
        i2c_master_cmd_begin(I2C_MASTER_NUM, cmd4, 1000 / portTICK_RATE_MS);
        i2c_cmd_link_delete(cmd4);
      }
    }
  }
...
void init(void) {
  ...
  gpio_isr_handler_add(BUTTON, timer_group0_isr, NULL);
...
void app_main(void) {
  ...
  xTaskCreate(timer_evt_task, "timer_evt_task", 1024*2, NULL, configMAX_PRIORITIES-1, NULL);
