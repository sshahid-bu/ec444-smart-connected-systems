...
// Assignment:  EC 444 Quest 0 Skill 08
// Skill:       Alphanumeric I2C Display 

// Adapted I2C example code to work with the Adafruit 14-segment Alphanumeric Display. 
// Key notes: MSB!!
...
static void test_alpha_display() {
    ...
    printf("Enter characters to display on I2C\n");
    while (count != fullInput) {
      int len = uart_read_bytes(EX_UART_NUM, userInput, BUF_SIZE, 20 / portTICK_RATE_MS);
      if (len > 0) {
        
        displaybuffer[count] = alphafonttable[( (int) userInput[0])];
        printf("Read: %c\n", userInput[0]);
        count ++;
      }
    ...
    while (1) {
      ...
      // if(ret == ESP_OK) { printf("- wrote: T.D.C.L. \n\n"); }
    }
